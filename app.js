...
if(!fs.existsSync('mkdir/data/contacts.json')){
  fs.writeFileSync('mkdir/data/contacts.json', '[]', 'utf-8');
}

//baca terlebih dahulu array yang ada di JSON
    fs.readFile('mkdir/data/contacts.json', 'utf-8', function readFileCallBack(err, data){
      if(err){
        ...
        //write array ke JSON
        fs.writeFile('mkdir/data/contacts.json', json, function callback(err){
          if(err){
            console.log(err);
          }
        });
